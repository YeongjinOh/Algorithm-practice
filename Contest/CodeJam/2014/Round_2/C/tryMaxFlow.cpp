...
//    memset(f,0,sizeof(f));
    ...
    // shrink cell
    /*
    vector<vector<bool> > shrink;
    shrink.push_back(cell[0]);
    for (int i=1; i<H; i++) {
        if (cell[i-1] != cell[i])
            shrink.push_back(cell[i]);
    }
    H = shrink.size();
    */
    ...
    v = sink+1;
    for (int i=0; i<v; i++) for (int j=0; j<v; j++) { c[i][j]=0; f[i][j]=0; }
    for (int i=0; i<H; i++) {
        for (int j=0; j<W; j++) {
            if (!cell[i][j]) continue;
            c[getFrom(i,j)][getTo(i,j)] = 1;
            if (j > 0)
                c[getTo(i,j)][getFrom(i,j-1)] = 1;
            if (j < W-1)
                c[getTo(i,j)][getFrom(i,j+1)] = 1;
            if (i < H-1)
                c[getTo(i,j)][getFrom(i+1,j)] = 1;
        }
    }
    for (int i=0; i<W; i++) {
        c[source][getFrom(0,i)] = 1;
        c[getTo(H-1,i)][sink] = 1;
    }
